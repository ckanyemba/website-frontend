...
import { sculpturesEdit } from "../../features/sculpturesSlice";
import { PrimaryButton } from "./CommonStyled";

export default function EditSculpture({ sculpId }) {
  const [open, setOpen] = React.useState(false);

  const dispatch = useDispatch();
  const { items, editStatus } = useSelector((state) => state.sculptures);
  ...
  const handleClickOpen = () => {
    ...
    let selectedProd = items.filter((item) => item._id === sculpId);
  ...
  return (
    <div>
      ...
      <Dialog
        ...
        maxWidth={"md"}
      >
        <DialogTitle>Edit sculpture </DialogTitle>
        <DialogContent>
          <StyledEditSculpture>
            ...
            <ImagePreview>
              {previewImg ? (
                <>
                  <img src={previewImg} alt="sculpture image!" />
                </>
              ) : (
...
const StyledEditSculpture = styled.div`
  display: flex;
